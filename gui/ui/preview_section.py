...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QFrame, QSplitter, QGroupBox, QScrollArea,
...
class PreviewWorker(QThread):
    ...
    def __init__(self, sample_text: str, excluded_terms: List[str], 
                 business_glossary: Dict[str, Dict[str, str]], target_language: str):
        super().__init__()
    ...
    def _generate_preview(self) -> Dict[str, str]:
        ...
        return {
            ...
            "excluded_terms_applied": len([term for term in self.excluded_terms if term.lower() in original_text.lower()]),
    ...
    def _apply_configuration(self, text: str) -> str:
        ...
        for term in self.excluded_terms:
            if term.lower() in result.lower():
                ...
                result, processed_positions = self._highlight_term_with_tracking(result, term, "[", "]", processed_positions)
        ...
        if self.target_language in self.business_glossary:
            ...
            for english_term, translation in glossary.items():
                if english_term.lower() in result.lower():
                    ...
                    result, processed_positions = self._highlight_term_with_tracking(result, english_term, "{", "}", processed_positions)
        
        return result
    
    def _highlight_term_with_tracking(self, text: str, term: str, prefix: str, suffix: str, processed_positions: set) -> tuple:
...
class PreviewSection(QWidget):
    ...
    def set_configuration(self, excluded_terms: List[str], business_glossary: Dict[str, Dict[str, str]]):
    ...
    def generate_preview(self):
        ...
        if not sample_text:
            QMessageBox.warning(self, "No Sample Text", 
                              "Please enter some sample text to preview.")
            return
    ...
    def on_preview_error(self, error_message: str):
        ...
        QMessageBox.warning(self, "Preview Error", 
                          f"Failed to generate preview: {error_message}")
