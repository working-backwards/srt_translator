...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QFrame, QGroupBox, QScrollArea, QProgressBar,
...
class ValidationWorker(QThread):
    ...
    def __init__(self, excluded_terms: List[str], business_glossary: Dict[str, Dict[str, str]], 
...
• Overall Quality: {results['quality']['score']:.1%} ({results['quality']['confidence']:.1%} confidence)"""
        ...
        if results["excluded_terms"]["issues"]:
            all_issues.extend([f"Excluded Terms: {issue}" for issue in results["excluded_terms"]["issues"]])
        if results["business_glossary"]["issues"]:
            all_issues.extend([f"Business Glossary: {issue}" for issue in results["business_glossary"]["issues"]])
        if results["quality"]["issues"]:
            all_issues.extend([f"Quality: {issue}" for issue in results["quality"]["issues"]])
        ...
        if results["excluded_terms"]["suggestions"]:
            all_suggestions.extend([f"• {suggestion}" for suggestion in results["excluded_terms"]["suggestions"]])
        if results["business_glossary"]["suggestions"]:
            all_suggestions.extend([f"• {suggestion}" for suggestion in results["business_glossary"]["suggestions"]])
        if results["quality"]["suggestions"]:
            all_suggestions.extend([f"• {suggestion}" for suggestion in results["quality"]["suggestions"]])
    ...
    def on_validation_error(self, error_message: str):
        ...
        QMessageBox.warning(self, "Validation Error", 
                          f"Failed to run validation: {error_message}")
