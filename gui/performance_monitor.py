...
class PerformanceMonitor(QObject):
    ...
    def __init__(self, max_history: int = 100):
        ...
        self.operation_stats: Dict[str, PerformanceStats] = defaultdict(
            PerformanceStats
        )
    ...
    def start_operation(self, operation: str, file_paths: List[str] = None):
        ...
        if self.current_operation:
            self.logger.warning(
                f"Starting operation '{operation}' while '{self.current_operation}' is still running"
            )
    ...
    def end_operation(
        self, operation: str, success: bool = True, error_message: str = ""
    ):
        ...
        self.logger.info(
            f"Completed operation: {operation} ({duration_ms:.1f}ms, {'success' if success else 'failed'})"
        )
    ...
    def _update_operation_stats(self, operation: str, metric: PerformanceMetric):
        ...
        total_memory = sum(
            m.memory_mb for m in self.metrics if m.operation == operation
        )
        ...
        if operation:
            ...
            return {
                ...
                "success_rate_percent": (
                    round(stats.successful_operations / stats.total_operations * 100, 1)
                    if stats.total_operations > 0
                    else 0
                ),
                "average_duration_ms": round(stats.average_duration_ms, 1),
                "min_duration_ms": (
                    round(stats.min_duration_ms, 1)
    ...
    def get_overall_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate_percent": round(
                successful_operations / total_operations * 100, 1
            ),
