...
class ConfigurationValidator:
    ...
    def validate_excluded_terms(self, excluded_terms: List[str], source_files: List[str]) -> ValidationResult:
        ...
        if total_terms == 0:
            return ValidationResult(
                ...
                suggestions=["Consider adding some excluded terms for better translation control"],
                confidence=0.5
            )
        ...
        confidence = min(validity_score + 0.2, 1.0)  # Boost confidence if most terms are valid
    ...
    def validate_business_glossary(self, business_glossary: Dict[str, Dict[str, str]], 
                                 source_files: List[str]) -> ValidationResult:
        ...
        if not business_glossary:
            return ValidationResult(
                ...
                suggestions=["Consider adding a business glossary for better translation consistency"],
                confidence=0.5
            )
        ...
        for language, terms in business_glossary.items():
            for english_term, translation in terms.items():
                ...
                if term_found:
                    ...
                    issues.append(f"Glossary term '{english_term}' not found in source files")
                    suggestions.append(f"Remove '{english_term}' from {language} glossary or add it to your content")
                
                # Check translation quality
                if not translation or translation.strip() == "":
                    issues.append(f"Empty translation for '{english_term}' in {language}")
                    suggestions.append(f"Provide a translation for '{english_term}' in {language}")
                elif translation.lower() == english_term.lower():
                    issues.append(f"Translation '{translation}' is identical to English term '{english_term}' in {language}")
                    suggestions.append(f"Consider providing a proper translation for '{english_term}' in {language}")
    ...
    def calculate_confidence_score(self, excluded_terms_result: ValidationResult, 
                                 glossary_result: ValidationResult) -> float:
    ...
    def validate_configuration_quality(self, excluded_terms: List[str], 
                                     business_glossary: Dict[str, Dict[str, str]]) -> ValidationResult:
        ...
        if len(excluded_terms) > 50:
            issues.append("Too many excluded terms (50+) may impact translation quality")
            suggestions.append("Consider reducing excluded terms to the most important ones")
        
        if len(excluded_terms) < 3:
            issues.append("Very few excluded terms may not provide enough control")
            suggestions.append("Consider adding more important terms to exclude from translation")
        ...
        for term in excluded_terms:
            if len(term) < 2:
                ...
                suggestions.append(f"Consider removing '{term}' or making it more specific")
    ...
    def get_validation_summary(self, excluded_terms: List[str], 
        ...
        glossary_validation = self.validate_business_glossary(business_glossary, source_files)
        quality_validation = self.validate_configuration_quality(excluded_terms, business_glossary)
        
        # Calculate overall confidence
        overall_confidence = self.calculate_confidence_score(excluded_validation, glossary_validation)
        
        # Determine overall status
        all_valid = (
            excluded_validation.is_valid and 
            glossary_validation.is_valid and 
