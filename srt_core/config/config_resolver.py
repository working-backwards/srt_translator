...
class CLIConfig:
    ...
    def from_environment(cls) -> "CLIConfig":
        ...
        required_vars = ["OPENAI_API_KEY", "TARGET_LANGUAGES"]
        ...
        try:
            ...
            logging.warning(
                f"Invalid DNT_TERMS format, using empty list: {dnt_terms_str}"
            )
            dnt_terms = []
        ...
        try:
            ...
            logging.warning(
                f"Invalid TERMBASE format, using empty dict: {termbase_str}"
            )
            termbase = {}
...
class ConfigResolver:
    ...
    def get_translation_config(
        ...
        if ConfigResolver.is_cli_mode():
            ...
            raise ValueError(
                "Cannot determine configuration source. Provide explicit parameters or ensure proper mode detection."
            )
