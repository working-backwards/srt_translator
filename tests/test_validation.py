...
class TestConfigurationValidator:
    ...
    def setup_method(self):
        ...
        with open(self.test_file1, 'w', encoding='utf-8') as f:
            f.write("Welcome to our business course. The CEO will discuss the company's API integration.")
        
        with open(self.test_file2, 'w', encoding='utf-8') as f:
            f.write("The CFO has prepared the quarterly report with Amazon Web Services.")
    ...
    def test_validate_excluded_terms_valid(self):
        ...
        result = self.validator.validate_excluded_terms(excluded_terms, self.source_files)
    ...
    def test_validate_business_glossary_valid(self):
        ...
        result = self.validator.validate_business_glossary(business_glossary, self.source_files)
        
        assert result.is_valid == True
        assert result.score == 1.0  # All terms found
        # Note: Issues will be flagged for identical translations, but score is still 1.0
        assert "CEO" in result.issues[0] or "CFO" in result.issues[0] or "API" in result.issues[0]
    ...
    def test_validate_configuration_quality_good(self):
        ...
        result = self.validator.validate_configuration_quality(excluded_terms, business_glossary)
    ...
    def test_calculate_confidence_score(self):
        ...
        confidence = self.validator.calculate_confidence_score(excluded_result, glossary_result)
