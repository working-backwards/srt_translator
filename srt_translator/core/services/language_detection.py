        ...
        if language_config and detected:
            try:
                if hasattr(language_config, "closest_supported_code"):
                    norm = language_config.closest_supported_code(detected)
                else:
                    supported = set(language_config.get_language_codes())
                    if detected in supported:
                        ...
                        if not norm and detected.lower().startswith("zh"):
                            norm = (
                                ...
                                else lowered.get("zh-hans")
                            )
                        if not norm and detected.lower().startswith("pt"):
                            norm = (
                                "pt-BR"
                                if "pt-BR" in supported
                                else lowered.get("pt-br")
                            )
                if norm:
                    name = language_config.get_language_name(norm)
            except Exception:
                norm, name = None, None
